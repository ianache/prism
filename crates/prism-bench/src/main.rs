...
fn s5_record(config: &prism_bench::cli::Config, dataset: &Dataset, host: &Metadata, metadata: &BTreeMap<String, String>, level: &str, window: &SustainedWindow, base_p99: Option<u128>) -> RawRecord {
    ...
    RawRecord {
        ...
        typed_rejections: window.typed_rejections, execution_failures: window.execution_failures, command: host.command.clone(), dataset_digest: dataset.manifest_digest.clone(), workflow_tax_percent: if level == "b1" { tax } else { 0.0 }, metadata: metadata.clone(), protocol_version: "1.1".into(), observability_variant: window.observability_variant.clone(), execution_id: window.execution_id.clone(),
